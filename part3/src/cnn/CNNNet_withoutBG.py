...
import numpy as np
from CNNDataset_withoutBG import create_window_label 
...
#         self.conv7 = nn.Conv2d(5*5*16, 3, 1)
...
class CnnNet(nn.Module):
    def __init__(self):
        ...
        self.conv5 = nn.Conv2d(256, 5*5*16, 5)
        self.conv6 = nn.Conv2d(5*5*16, 5*5*16, 1)
        self.conv7 = nn.Conv2d(5*5*16, 3, 1)
        ...
        for i, (imgs, annotations, _) in enumerate(trainloader):
            ...
            if loss < best_losses:
                ...
                torch.save(net.state_dict(),
                            '../../data/data2/CNNNet/withoutBG/checkpoints/model-epoch-{}-losses-{:.8f}.pth'.format(epoch + 1, loss))
                # print('done saving')
...
def CNN_testing(net, images, obj_thred):
