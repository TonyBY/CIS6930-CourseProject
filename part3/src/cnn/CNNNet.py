...
class CnnNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(x.size())
        result = nn.Softmax(dim = 1)(F.relu(self.conv4(x)))
        return result
...
def CNN_taining(trainloader,net, device, criterion, optimizer):
    ...
    for epoch in range(2):  # loop over the dataset multiple times
        ...
        for i, (imgs, annotations) in enumerate(trainloader):
            ...
            gt = create_window_label(annotations)
            ...
            if loss < best_losses:
                ...
                torch.save(net.state_dict(),
                            '../data/data2/CNNNet/checkpoints/model-epoch-{}-losses-{:.8f}.pth'.format(epoch + 1, loss))
                print('done saving')
